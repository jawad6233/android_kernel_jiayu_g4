...
#ifdef VENDOR_EDIT
    char name[] = "ZsdScaladoHdrShot";
    if (memcmp(pszShotName, name, sizeof(name)) == 0) {
        CAM_LOGD("[%s] ZsdScaladoHdrShot", __FUNCTION__);
        pImpShot = new ZsdScaladoHdrShot(pszShotName, u4ShotMode, i4OpenId);
    } else {
...
onYuv_alloc(ImgBufInfo   & rJpgImgBufInfo, ImgBufInfo & rYuvImgBufInfo, ImgBufInfo & rThumbImgBufInfo)
{
    ...
    /* xinlan.he@Prd.Camera.CameraSrv for: here need larger memory */
    #ifdef VENDOR_EDIT
...
ZsdScaladoHdrShot(
    ...
    : ZsdShot(pszShotName, u4ShotMode, i4OpenId)
{
}
 ...
 *
 ******************************************************************************/
bool ZsdScaladoHdrShot::onCmd_capture()
{
    MBOOL ret = MTRUE;
    //
    CPTLog(Event_ZsdShot_capture, CPTFlagStart);
    ...
    mImageIdx = 0;
    for (int i=0; i<3; i++) {
        if (mpCaptureBufHandler == NULL) {
            MY_LOGE("mpCaptureBufHandler is NULL !!");
        }
        mpCaptureBufHandler->dequeProcessor(rNode, mpCaptureBufHandler->getStoredBufferCnt()-1);
        CPTLogStrEx(Event_ZsdShot_capture, CPTFlagSeparator, (MUINT32)rNode.mainImgNode->getVirAddr(), 0, "dequeProcessor");
...
*
*******************************************************************************/
MBOOL ZsdScaladoHdrShot::handleJpegData(MUINT8* const puJpegBuf, MUINT32 const u4JpegSize, MUINT8* const puThumbBuf, MUINT32 const u4ThumbSize)
{
    AutoCPTLog cptlog(Event_ZsdShot_handleJpegData);
    MY_LOGD("+ (puJpgBuf, jpgSize, puThumbBuf, thumbSize) = (%p, %d, %p, %d)", puJpegBuf, u4JpegSize, puThumbBuf, u4ThumbSize);
