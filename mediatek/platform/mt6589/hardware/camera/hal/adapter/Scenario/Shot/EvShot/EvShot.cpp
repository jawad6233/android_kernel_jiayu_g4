...
#ifdef VENDOR_EDIT
    char name[] = "EvShotCustom";
    if (memcmp(pszShotName, name, sizeof(name)) == 0) {
        CAM_LOGD("[%s] EvShotCustom", __FUNCTION__);
        pImpShot = new EvShotCustom(pszShotName, u4ShotMode, i4OpenId);
    } else {
...
/* xinlan.he@Prd.Camera.CameraSrv add */
#ifdef VENDOR_EDIT
/******************************************************************************
 *
 ******************************************************************************/
...
EvShotCustom(
    ...
    : EvShot(pszShotName, u4ShotMode, i4OpenId)
    , mu4CurrentCount(0)
    , mfgIsCanceled(MFALSE)
{
}
...
{ 
    ...
    //
    pSingleShot->setCallbacks(EvShot::fgCamShotNotifyCb, EvShot::fgCamShotDataCb, this); 
    //     
    ret = pSingleShot->setShotParam(rShotParam); 
