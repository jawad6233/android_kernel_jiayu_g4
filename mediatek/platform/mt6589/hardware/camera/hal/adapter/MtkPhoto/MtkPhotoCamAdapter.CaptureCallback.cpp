...
#ifdef VENDOR_EDIT
//zhiquan.huang@MMApp.Camera, 2013-03-28 Add for abandon the postview of normalshot except for continuoushot
#include "caps_base.h"
#endif
...
#ifdef VENDOR_EDIT
//zhiquan.huang@MMApp.Camera, 2013-04-16 Add for resizepostview
MBOOL
CamAdapter::
isResizePostview(uint32_t const  u4PostviewSize, uint32_t const  u4ImgBufSize){
#ifdef  CAM_DRV_13009
		return false;
#else
		switch(mpParamsMgr->getOpenId()){
		    case 0:
				MY_LOGD("[isResizePostview] mPostviewClientFormat = %s", mPostviewClientFormat.string());
                if(mPostviewClientFormat == CameraParameters::PIXEL_FORMAT_YUV420SP){
                    int iPreviewWidth = 0, iPreviewHeight = 0;
                    mpParamsMgr->getPreviewSize(&iPreviewWidth, &iPreviewHeight);
					MY_LOGD("[isResizePostview] mPostviewHeight, u4ImgBufSize, iPreviewHeight, u4PostviewSize (%d, %d, %d, %d)", mPostviewHeight, u4ImgBufSize, iPreviewHeight, u4PostviewSize);
                    return mPostviewHeight*u4ImgBufSize == iPreviewHeight*u4PostviewSize;
                }
                return false;
		    case 1:
		        return false;
		    default:
		        MY_LOGE("[isResizePostview] invalid cameraId =%d not handled", mpParamsMgr->getOpenId());
				return false;
	}
#endif
}
...
/* oppo xinlan.he@Prd.Camera.CameraSrv modify */
#ifdef VENDOR_EDIT
    if ((getParamsManager()->getShotMode() == eShotMode_docSkinbeauty) \
                || (getParamsManager()->getShotMode() == eShotMode_scaladoLomo) \
                || (getParamsManager()->getShotMode() == eShotMode_NormalShot)) //wuhongru add scaladolomo and normal
    {
        MY_LOGD("docSkinBeauty and scaladoLomo and normal should not show the postview display!");
        return  false; 
    }
#endif
	...
	//{
		//static int i = 0;
		//sprintf(filename, "mnt/sdcard/previewhi/yuvpostview_begin_%d.yuv", i);
		//saveBufToFile(filename, (MUINT8*)puPostviewBuf, u4PostviewSize);
    //}
...
#ifdef VENDOR_EDIT
//zhiquan.huang@MMApp.Camera, 2013-04-16 Add for resizepostview
		if(isResizePostview(u4PostviewSize, pImgBuf->getBufSize())){
            MY_LOGW("memcpy calc begin");
            int postYDataSize = u4PostviewSize * 2 / 3;
            int dispYDataSize = pImgBuf->getBufSize() * 2 / 3;

            int postUVDataSize = u4PostviewSize  / 6; 
            int dispUVDataSize = pImgBuf->getBufSize() / 6;
            MY_LOGW("memcpy begin");
            ::memcpy(pImgBuf->getVirAddr(), puPostviewBuf + (postYDataSize - dispYDataSize) / 2, dispYDataSize);
            ::memcpy(pImgBuf->getVirAddr() + dispYDataSize, 
                    puPostviewBuf + postYDataSize + (postUVDataSize - dispUVDataSize) / 2,
                    dispUVDataSize);
            ::memcpy(pImgBuf->getVirAddr() + dispYDataSize + dispUVDataSize, 
                    puPostviewBuf + postYDataSize + postUVDataSize + (postUVDataSize - dispUVDataSize) / 2,
                    dispUVDataSize);
 ...
 * ===  FUNCTION  ======================================================================
 *         Name:  resizePostview
 *  Description:  resize the postview to fit the ratio of preview-size, also
 *  prevent the stretch phenomenon
 *  the procedure of postview is  postview(1280*720)-> stretch(1280*960) -> crop(1280*720)
 *  the present format of postview is yuv420 yv12
 * =====================================================================================
 */
void CamAdapter::resizePostview(void  *mem ) {
    MY_LOGD("CamAdapter resize postview start"); 
    //int i = 0;
    //char filename[256] = { '\0'};
    //char filename1[256] = {'\0'};
    //i++;
    CTSize inDims, outDims;
    CTBuffer inBuffer, outBuffer, cropBuffer;
    CTint32 fullW, fullH;
    CTImage *pInImage;
    CTImage *pOutImage;
    CTImage *pCropImage;
    ...
    int iPreviewWidth = 0, iPreviewHeight = 0;
    mpParamsMgr->getPreviewSize(&iPreviewWidth, &iPreviewHeight);
    inDims.w = iPreviewWidth;
    inDims.h = iPreviewHeight;
    cropSize.w = inDims.w;
    cropSize.h = inDims.h;

    int iPictureWidth = 0, iPictureHeight = 0;
    mpParamsMgr->getPictureSize(&iPictureWidth, &iPictureHeight);
    if( iPictureWidth*iPreviewHeight == iPictureHeight*iPreviewWidth ){
        MY_LOGD("resize previewsize and picture size remain the same, it didn't need to adjust"); 
        return;
    }

    if(inDims.w > inDims.h){
        outDims.w = inDims.w;
        outDims.h = iPictureHeight*inDims.w/iPictureWidth;
    }else{
        outDims.h = inDims.h;
        outDims.w = iPictureWidth*inDims.h/iPictureHeight;
    }
    MY_LOGD("inDims.w =%d, inDims.h =%d outDims.w = %d, outDims.h = %d",inDims.w, inDims.h, outDims.w, outDims.h); 

    if(!mPostviewResizeMem){
        mPostviewResizeMem = ::malloc(outDims.w*outDims.h*3/2);
    }

    //sprintf(filename, "mnt/sdcard/yuvpostview_begin_%d.yuv", i);
    //saveBufToFile(filename, (MUINT8*)mem, 1280*720*1.5); 

    inBuffer.size = inDims.w*inDims.h*3/2;
    inBuffer.data = (CTuint8*)(mem);
    outBuffer.size = outDims.w*outDims.h*3/2;
    outBuffer.data = (CTuint8*)mPostviewResizeMem;
    cropBuffer.size = inBuffer.size;
    cropBuffer.data = (CTuint8*)mem;
    //resize
    caps_createImage(&inBuffer, inDims, inDims.w*3/2, CM_YUV420P, &pInImage);
    caps_newSessionWithRaw(pInImage, &session);
    caps_createImage(&outBuffer, outDims, outDims.w*3/2, CM_YUV420P, &pOutImage);
    caps_renderAsRawToBuffer(session, pOutImage, CAPS_OUTPUT_STRETCH, 0);
    //sprintf(filename1, "mnt/sdcard/yuvpostview_after_%d.yuv", i);
    //MY_LOGD("filename1 = %s, puBuf =%d, u4size = %d", filename1, mPostviewResizeMem, 1280*960*1.5);
    //saveBufToFile(filename1, (MUINT8*)mPostviewResizeMem, 1280*960*1.5);
    caps_destroySession(session);
   
    //crop
    cropArea.w = inDims.w;
    cropArea.h = inDims.h;
    cropArea.x = (outDims.w - cropArea.w) / 2;
    cropArea.y = (outDims.h - cropArea.h) / 2;
    
    //MY_LOGD("cropArea.x = %d cropArea.y =%d cropArea.w=%d cropArea.h=%d",cropArea.x, cropArea.y, cropArea.w, cropArea.h);
    caps_newSessionWithRaw(pOutImage, &session);
    caps_doCrop(session, cropArea);
    
    caps_createImage(&cropBuffer, inDims, inDims.w*3/2, CM_YUV420P, &pCropImage);
    caps_renderAsRawToBuffer(session, pCropImage, CAPS_OUTPUT_STRETCH, 0);
    caps_destroySession(session);

    MY_LOGD("CamAdapter resize postview end"); 
}
#endif /* VENDOR_EDIT */
...
)
{
    ...
    if  ( msgTypeEnabled(CAMERA_MSG_POSTVIEW_FRAME) )
    {
		...
		MY_LOGD("CAMERA_MSG_POSTVIEW_FRAME\n");
		if(strcmp(mpParamsMgr->getStr(MtkCameraParameters::QUICK_CAPTURE_SUPPORTED),"off") == 0)
	    {  MY_LOGD("onCB_PostviewDisplay\n");
	       onCB_PostviewDisplay(i8Timestamp,u4PostviewSize,puPostviewBuf);
	    }
    }
	#endif /*QUICK_CAPTURE*/
	#endif /*VENDOR_EDIT*/
