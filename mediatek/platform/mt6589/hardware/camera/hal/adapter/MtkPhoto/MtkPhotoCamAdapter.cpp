...
#ifdef VENDOR_EDIT
    for (int i=0; i<CUSTOM_CNT; i++) {
        mpCustomProcessClient[i] = ICustomProcess::createInstance(mpParamsMgr, 
                                                               mpPreviewBufMgr, 
                                                               getOpenId(),
                                                               IImgBufProvider::eID_CUSTOM + i, 
                                                               "CustomProcess@ImgBufQue",
                                                               1);
        if (mpCustomProcessClient[i] != NULL)
        {
            if (!mpCustomProcessClient[i]->init())
            {
                MY_LOGE("mpCustomProcessClient[%d]->Init fail", i);
                goto lbExit;            
            }
            if (!mpCustomProcessClient[i]->setImgBufProviderClient(this))
            {
                MY_LOGE("mpCustomProcessClient[%d]->setImgBufProviderClient fail", i);
                goto lbExit;            
            }
        }
    }
...
/* xinlan.he@Prd.Camera.CameraSrv for: CUSTOM_PROCESS */
    ...
    for (int i=0; i<CUSTOM_CNT; i++) {
        if (mpCustomProcessClient[i] != NULL)
        {
            if (!mpCustomProcessClient[i]->uninit())
            {
                MY_LOGE("mpCustomProcessClient[%d]->uninit fail", i);
            }
            mpCustomProcessClient[i] = NULL;
        }
    }
