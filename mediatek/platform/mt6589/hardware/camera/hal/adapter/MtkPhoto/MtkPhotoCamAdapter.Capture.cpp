...
#ifdef VENDOR_EDIT
    for (int i=0; i<CUSTOM_CNT; i++) {
        mpCustomProcessClient[i]->takePicture();
    }
#endif
...
onCaptureThreadLoop()
{
	...
	#ifdef QUICK_CAPTURE
    //lanhe@MTKcamera.captrue add for speedup camera captrue
    //if(preview_sem_inited == 0)
    {
		::sem_init(&preview_sem, 0, 0);
		//preview_sem_inited = 1;
    }
	#endif /*QUICK_CAPTURE*/
    #endif /*VENDOR_EDIT*/
    if  ( ! ret || pShot == 0 )
...
#warning "[TODO] perform a dummy compressed-image callback or CAMERA_MSG_ERROR to inform app of end of capture?"
    ...
    else
    {
		...
		#else
		#ifndef QUICK_CAPTURE
        shotParam.ms8PostviewDisplayFormat  = s8DisplayFormat;
        shotParam.ms8PostviewClientFormat   = pParamsMgr->getStr(MtkCameraParameters::KEY_PREVIEW_FORMAT);
		#else
		if((strcmp(mpParamsMgr->getStr(MtkCameraParameters::QUICK_CAPTURE_SUPPORTED),"on") == 0)&&(pParamsMgr->getInt(MtkCameraParameters::KEY_BURST_SHOT_NUM)<=1))
		{
	        shotParam.ms8PostviewDisplayFormat  = CameraParameters::PIXEL_FORMAT_RGB565;
	        shotParam.ms8PostviewClientFormat   = CameraParameters::PIXEL_FORMAT_RGB565;
		}else{
			shotParam.ms8PostviewDisplayFormat  = s8DisplayFormat;
            shotParam.ms8PostviewClientFormat   = pParamsMgr->getStr(CameraParameters::KEY_PREVIEW_FORMAT);
		}
		...
		#ifndef VENDOR_EDIT
        //zhiquan.huang@MMApp.Camera, 2013-04-23 Add for yv12 postview resize
		...
		if(mpParamsMgr->getOpenId() == 0){
			shotParam.mi4PostviewHeight         = iPreviewWidth*iPictureHeight / iPictureWidth;	
		}else{
			shotParam.mi4PostviewHeight         = iPreviewHeight;
		}
		#endif
        ...
        #ifdef VENDOR_EDIT
        //zhiquan.huang@MMApp.Camera, 2013-04-23 Add for yv12 postview resize
        mPostviewClientFormat = shotParam.ms8PostviewClientFormat;
        mPostviewHeight = shotParam.mi4PostviewHeight;
        #endif /* VENDOR_EDIT */
        //
