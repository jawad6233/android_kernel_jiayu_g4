...
)
{
	...
	//ZhangZhengrong@MtkCamera, 2013/03/19, Add for scene detect debug
    if(_msgType == MTK_CAMERA_MSG_EXT_NOTIFY &&
        _ext1 == MTK_CAMERA_MSG_EXT_NOTIFY_ASD)
    {
        CAM_LOGD("doNotifyCb(MTK_CAMERA_MSG_EXT_NOTIFY)");
        mspCamMsgCbInfo->mNotifyCb(MTK_CAMERA_MSG_EXT_NOTIFY, _ext1, _ext2, mspCamMsgCbInfo->mCbCookie);
    }
