...
precap()
{
    ...
    /* oppo xinlan.he@Prd.Camera.CameraSrv modify 
       *  In case of the endless circle, and the cnt will not reach 100 in the normal case. 
       */
    #ifdef VENDOR_EDIT
         updateCnt++;
         if (updateCnt > 20) //wuhongru@CamSrv decrease waiting time
         {
            MY_LOGE("Something is wrong!! updateCnt(%d)",updateCnt);
            break;
         }
    #endif
    }
...
/* xinlan.he@Prd.Camera.CameraSrv for: CUSTOM_PROCESS */
#ifdef VENDOR_EDIT
    if ((getParamsMgr()->getShotMode() == eShotMode_scaladoLomo) ||(getParamsMgr()->getShotMode() == eShotMode_docSkinbeauty))
    {
        mspPreviewBufHandler->dequeBuffer(eID_Pass2Custom, dispNode);
    } 
    else 
    {
    ...
    if ( dispNode.getImgBuf() != 0)
    {
       ...
       #ifdef OPPO_EXT_ISP_MS2R //OPPO_12083_MS2R_ISP
	//ZhangZhengrong@MtkCamera, 2013/03/19, Add for scene detect
	mp3AHal->PictureProcessing((MUINT8*)(dispNode.getImgBuf()->getVirAddr()),
		ImgInfo.u4Width, ImgInfo.u4Height);
