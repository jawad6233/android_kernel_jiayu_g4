...
dequeBuffer(int ePort, ImgBufQueNode &node)
{
    ...
    /* xinlan.he@Prd.Camera.CameraSrv for: CUSTOM_PROCESS */
    #ifdef VENDOR_EDIT
        case eID_Pass2Custom:
        {
            sp<IImgBufProvider> bufProvider = mspImgBufProvidersMgr->getCustomPvdr();
            if (bufProvider != 0 && bufProvider->dequeProvider(node))
            {
                //MY_LOGD("getCustom1Pvdr get a node mIdx=%d", mIdx);
                node.setCookieDE(eBuf_Custom);
                node.setCookieED(mIdx++);
                ret = true;
                break;
            }

            bufProvider = mspImgBufProvidersMgr->getCustom2Pvdr();
            if (bufProvider != 0 && bufProvider->dequeProvider(node))
            {
                //MY_LOGD("getCustom2Pvdr get a node mIdx=%d", mIdx);
                node.setCookieDE(eBuf_Custom2);
                node.setCookieED(mIdx++);
                ret = true;
                break;
            }
            bufProvider = mspImgBufProvidersMgr->getCustom3Pvdr();
            if (bufProvider != 0 && bufProvider->dequeProvider(node))
            {
                //MY_LOGD("getCustom3Pvdr get a node mIdx=%d", mIdx);
                node.setCookieDE(eBuf_Custom3);
                node.setCookieED(mIdx++);
                ret = true;
                break;
            }
            bufProvider = mspImgBufProvidersMgr->getCustom4Pvdr();
            if (bufProvider != 0 && bufProvider->dequeProvider(node))
            {
                //MY_LOGD("getCustom4Pvdr get a node mIdx=%d", mIdx);
                node.setCookieDE(eBuf_Custom4);
                node.setCookieED(mIdx++);
                ret = true;
                break;
            }
            MY_LOGD("getCustomPvdr deque a node fail!");
        }
        break;
    #endif
        //
...
enqueBuffer(ImgBufQueNode const& node)
{
    ...
    /* xinlan.he@Prd.Camera.CameraSrv for: CUSTOM_PROCESS */
        ...
        case eBuf_Custom:
        {
            ...
            if (bufProvider != 0)
            {
                //MY_LOGD("getCustom1Pvdr enque a node");
                bufProvider->enqueProvider(node);
            }
        }
        break;
        ...
        case eBuf_Custom2:
        {
            sp<IImgBufProvider> bufProvider = mspImgBufProvidersMgr->getCustom2Pvdr();
            if (bufProvider != 0)
            {
                //MY_LOGD("getCustom2Pvdr enque a node");
                bufProvider->enqueProvider(node);
            }
        }
        break;
        ...
        case eBuf_Custom3:
        {
            sp<IImgBufProvider> bufProvider = mspImgBufProvidersMgr->getCustom3Pvdr();
            if (bufProvider != 0)
            {
                //MY_LOGD("getCustom3Pvdr enque a node");
                bufProvider->enqueProvider(node);
            }
        }
        break;
        ...
        case eBuf_Custom4:
        {
            sp<IImgBufProvider> bufProvider = mspImgBufProvidersMgr->getCustom4Pvdr();
            if (bufProvider != 0)
            {
                //MY_LOGD("getCustom4Pvdr enque a node");
                bufProvider->enqueProvider(node);
            }
        }
        break;
