...
//zhiquan.huang@MMApp.Camera, 2013-03-28 Add for abandon the postview of normalshot except for continuoushot
    void                            resizePostview(void *mem);
    MBOOL                           isResizePostview(uint32_t const  u4PostviewSize, uint32_t const  u4ImgBufSize);
#endif /* VENDOR_EDIT */
...
/* xinlan.he@Prd.Camera.CameraSrv for: CUSTOM_PROCESS */
#ifdef VENDOR_EDIT
    enum {CUSTOM_CNT = 4}; // the CUSTOM_CNT must less than 4
    sp<ICustomProcess>              mpCustomProcessClient[CUSTOM_CNT];
#endif

#ifdef VENDOR_EDIT
//zhiquan.huang@MMApp.Camera, 2013-03-28 Add for abandon the postview of normalshot except for continuoushot
    void*                           mPostviewResizeMem;
    String8     	     			mPostviewClientFormat; 
    int                             mPostviewHeight;
#endif
};
