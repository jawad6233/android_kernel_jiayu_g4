...
onHandleCapture()
{
    ...
    #ifdef VENDOR_EDIT
        if (mu4ShotMode == eShotMode_scaladoHdr) {
            if ( ! pPreviewCmdQueThread->postCommand(PrvCmdCookie::eCaptureUpdate, PrvCmdCookie::eSemAfter) ) {
                MY_LOGE("eShotMode_scaladoHdr:onHandleCapture capture preview update: fail");
            }
        } else {
...
#warning "[TODO] perform a dummy compressed-image callback or CAMERA_MSG_ERROR to inform app of end of capture?"
    ...
    /* xinlan.he@Prd.Camera.CameraSrv for: scalado_hdr */
    #ifdef VENDOR_EDIT
        || mu4ShotMode == eShotMode_scaladoHdr
    #endif
        || mu4ShotMode == eShotMode_ContinuousShotCc) {
