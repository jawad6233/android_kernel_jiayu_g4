...
/* xinlan.he@Prd.Camera.CameraSrv for: scalado_hdr */
#ifdef VENDOR_EDIT
    bool                update3FrameForScaladoHdr();
#endif
...
PreviewCmdQueThread::threadLoop()
{
    ...
    if (getCommand(pCmdCookie))
    {
        ...
        switch (pCmdCookie->getCmd())
        {
            ...
            /* oppo xinlan.he@Prd.Camera.CameraSrv for: scalado_hdr */
            #ifdef VENDOR_EDIT
			    #ifdef OPPO_EXT_ISP_MS2R
			    if (mShotMode != eShotMode_scaladoHdr)
                {
                    isvalid = update();
                }
                #else
...
delay(EQueryType_T type)
{
    ...
    switch (type)
    {
        ...
        /* oppo xinlan.he@Prd.Camera.CameraSrv for: scalado_hdr */
		    ...
		    if (mShotMode == eShotMode_scaladoHdr)
		    {
		        MY_LOGD("hxl do not to preview, just to capture");
                return true;
            }
		#endif
		#endif
...
#ifdef VENDOR_EDIT
#ifdef OPPO_EXT_ISP_MS2R
        mShotMode = getParamsMgr()->getShotMode();
...
update3FrameForScaladoHdr()
{
    ...
    MUINT32 sceneModeReserve;
    if ( ! mp3AHal->getParams(cam3aParam) )
    {
        MY_LOGE("scalado hdr: getParams fail");
        ret = false;
    }

    sceneModeReserve = cam3aParam.u4SceneMode;
    cam3aParam.u4SceneMode = SCENE_MODE_HDR;
    if ( ! mp3AHal->setParams(cam3aParam) ) 
    {
        MY_LOGE("setParams fail");
        ret = false;
    }
    dropFrame();
    MY_LOGD("start get 3 frame");
    for (j=0; j<frameNum; j++)
    {
        if (!updateOne()) 
        {
            ret = false;
            MY_LOGE("get 3 frame fail");
            break;
        }
    }
    
    cam3aParam.u4SceneMode = sceneModeReserve;
    if ( ! mp3AHal->setParams(cam3aParam) ) 
    {
        MY_LOGE("setParams fail 2");
        ret = false;
    }
#else 
    int i = 0, dropFrameNum = 5;
    MINT32 expIndexOffset[3] = {0, 2, -2};
    NS3A::Param_T cam3aParam;
    MINT32 currExpIndex;
    ...
    currExpIndex = cam3aParam.i4ExpIndex; 
    for (j=0; j<frameNum && ret; j++) 
    {
        MY_LOGD("the %d frame begin", j);
        if (expIndexOffset[j] != 0) 
        {
            cam3aParam.i4ExpIndex = currExpIndex + expIndexOffset[j];
            if ( ! mp3AHal->setParams(cam3aParam) ) 
            {
                MY_LOGE("setParams fail");
                ret = false;
        ...
        MY_LOGD("the %d frame end", j);
    }
    
    cam3aParam.i4ExpIndex = currExpIndex;
    if ( ! mp3AHal->setParams(cam3aParam) ) 
    {
        MY_LOGE("setParams fail");
        ret = false;
...
#ifdef VENDOR_EDIT
     if (mShotMode == eShotMode_scaladoHdr 
        && IStateManager::inst()->isState(IState::eState_PreCapture)) 
    {
        if (!update3FrameForScaladoHdr())
        {
            MY_LOGW("get 3 frame failed!!");
        }
    }
#endif
...
/* xinlan.he@Prd.Camera.CameraSrv for: scalado_hdr */	
#ifdef VENDOR_EDIT
    && mShotMode != eShotMode_scaladoHdr 
#endif
