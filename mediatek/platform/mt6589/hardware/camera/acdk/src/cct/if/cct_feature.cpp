...
)
{
	...
	case ACDK_CCT_OP_FLASH_ENABLE:
        err = m_pSensorHalObj->sendCommand(SENSOR_DEV_MAIN, SENSOR_CMD_SET_FLASH_MODE, (int)FLASHLIGHT_TORCH, 0, 0);
        break;
	case ACDK_CCT_OP_FLASH_DISABLE:
		err = m_pSensorHalObj->sendCommand(SENSOR_DEV_MAIN, SENSOR_CMD_SET_FLASH_MODE, (int)FLASHLIGHT_FORCE_OFF, 0, 0);
		break;
	#else
   	case ACDK_CCT_OP_FLASH_ENABLE:
