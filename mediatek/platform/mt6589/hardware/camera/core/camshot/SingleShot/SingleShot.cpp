...
createSensorRawImg(SensorParam const & rSensorParam, ImgBufInfo const & rRawImgBufInfo)
{
    ...
    #ifdef VENDOR_EDIT
	#ifdef QUICK_CAPTURE
	//lanhe@MTKcamera.capture add for speedup capture
	mpCamIOPipe->startOne();
	#endif
    #endif /*VENDOR_EDIT*/
    profile.print();
	#ifdef OPPO_EXT_ISP_MS2R //OPPO_12083_MS2R_ISP  replace  macro
	char value[32] = {'\0'}; 
	MUINT32 mu4DumpRawFlag = 0;
    property_get("persist.isp.tuning.capture.raw", value, "0");
	mu4DumpRawFlag = ::atoi(value);
    if (mu4DumpRawFlag)
    {
        char fileName[256] = {'\0'}; 
		static MUINT32 mu4RawDataCnt = 0;
		mu4RawDataCnt++;
        sprintf(fileName, "/%s/shot_raw%dx%d_%d.raw", MEDIA_PATH, rRawImgBufInfo.u4ImgWidth, rRawImgBufInfo.u4ImgHeight, mu4RawDataCnt); 
