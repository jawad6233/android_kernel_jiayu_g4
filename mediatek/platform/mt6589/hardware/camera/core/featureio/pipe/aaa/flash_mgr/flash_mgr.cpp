...
#ifdef VENDOR_EDIT
//zhangkw@CameraDrv.flashlight, 2013/03/13, add for Torch
static int is_StrobeOn = 0;
#endif /*VENDOR_EDIT*/
//====================================================
// function
...
void hw_isoToGain(int iso, int* afe, int* isp)
{
	LogInfo("hw_isoToGain line %d +",__LINE__);
	...
	LogInfo("line=%d hw_isoToGain=iso=%d gain(x1000)=%d",__LINE__, iso, (int)(g*1000));
	...
	LogInfo("dev_1xGainIso %d", (int)devInfo.u4MiniISOGain);
	LogInfo("dev minG, amxG %d %d", (int)devInfo.u4MinGain, (int)devInfo.u4MaxGain);
	LogInfo("line=%d hw_isoToGain iso=%d afe=%d isp=%d",__LINE__, iso, *afe, *isp);
	LogInfo("hw_isoToGain line %d -",__LINE__);
#endif
}
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
void hw_capIsoToGain(int iso, int* afe, int* isp)
{
	...
	LogInfo("line=%d hw_capIsoToGain=iso=%d gain(x1000)=%d",__LINE__, iso, (int)(g*1000));
	...
	LogInfo("hw_capIsoToGain iso=%d afe=%d isp=%d (a(x1000)=%d)", iso, *afe, *isp, (int)(g*1000));
#endif
}
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
...
void FlashMgr::setAFLampOnOff(int en)
{
	...
	if(en==1)
	{
		...
		g_pStrobe->setTimeOutTime(0); //0 @zhangkw modify
		g_pStrobe->setStep(step);
		g_pStrobe->setDuty(duty);
	}
	else
...
int FlashMgr::isNeedFiringFlash()
{
    ...
    #ifdef VENDOR_EDIT
    //zhangkw@CameraDrv.flashligh, 2013/03/13, add for Torch
    LogInfo("[isNeedFiringFlash] print m_flashMode:%d\n", m_flashMode);
    if( m_flashMode==LIB3A_FLASH_MODE_FORCE_TORCH)
    {
    	LogInfo("isNeedFiringFlash() line=%d flash mode = Torch",__LINE__);
    	bFlashOn=0;
...
LogInfo("run() line=%d",__LINE__);
	...
	//convert flash3A
	hw_convert3ASta(&staData, para->staBuf);
LogInfo("run() hw_convert3ASta- line=%d",__LINE__);

	int isNext;
	FlashAlgExpPara paraNext;
	pStrobeAlg->AddStaData10(&staData, &g_expPara, &isNext, &paraNext);
LogInfo("run() AddStaData10- line=%d",__LINE__);
	...
	{
		rep->isEnd=1;
LogInfo("run() Estimate+ line=%d %d %d",__LINE__,m_iteration,isNext);
		pStrobeAlg->Estimate(&g_expPara);
LogInfo("run() Estimate- line=%d",__LINE__);

		int afe;
		int isp;
		hw_capIsoToGain(g_expPara.iso, &afe, &isp);
LogInfo("run() hw_capIsoToGain- line=%d",__LINE__);
...
int FlashMgr::setFlashMode(int mode)
{
	...
	#ifdef VENDOR_EDIT
	//zhangkw@CameraDrv.flashlight, 2013/03/13, add for torch
	if(is_StrobeOn ==1 && mode == 4) {
		LogInfo("setFlashMode The Torch is already on, just return!!\n");
		return 0;
	}
	#endif /*VENDOR_EDIT*/
	...
	else
	{
		...
		{
			#ifdef VENDOR_EDIT
			//zhangkw@CameraDrv.flashlight, 2013/0313, modify for Torch
			is_StrobeOn = 1;
			LogInfo("setFlashMode FLASHLIGHT_TORCH line==%d\n", __LINE__);
			#endif /*VENDOR_EDIT*/
			FLASH_PROJECT_PARA prjPara;
			loadNvram();
			int aeMode;
		...
		{
			#ifdef VENDOR_EDIT
			//zhangkw@CameraDrv.flashlight, 2013/03/13, modify for Torch
			is_StrobeOn = 0;
			#endif /*VENDOR_EDIT*/
			g_pStrobe = StrobeDrv::createInstance();
			g_pStrobe->setTimeOutTime(1000);
...
LogWarning("capture flash timeout line=%d",__LINE__);
	...
	#ifdef VENDOR_EDIT
	//zhangkw@CameraDrv.flashligh, 2013/03/13, Don't turn off flash, when Torch mode
	if(is_StrobeOn == 1)
	{
		LogInfo("Because Torch on, so does not turn off flash\n");
		m_flashMode = 4;
		return 0;
	}
	#endif /*VENDOR_EDIT*/
