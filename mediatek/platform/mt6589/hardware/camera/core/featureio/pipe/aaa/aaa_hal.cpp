...
MBOOL Hal3A::cancelAutoFocus()
{
    ...
    #ifdef VENDOR_EDIT
    //zhangkw@CameraDrv.camera, 2013/04/17, when cancel touch_af, shutdown af lamp
    if((FlashMgr::getInstance()->getFlashMode()) != 4)
		FlashMgr::getInstance()->setFlashOnOff(0);
    #endif /*VENDOR_EDIT*/
    return MTRUE;
}
...
#ifdef VENDOR_EDIT
//zhangkw@CameraDrv.camera, 2013/04/25, modify for issue that , when torch mode, used flash will be show in exif info.
    if(FlashMgr::getInstance()->isFlashOnCapture() || (FlashMgr::getInstance()->getFlashMode()) ==4)
#else
