...
ImgSensorDrv::impSearchSensor(pfExIdChk pExIdChkCbf)
{
  ...
  // #endif     
	    ...
	    #ifdef VENDOR_EDIT
	     //zhangkw@CameraDrv.camera, 2013/03/18, modify for camera
	     	if(id[KDIMGSENSOR_INVOKE_DRIVER_0]==0x20000) 
			{
		       LOG_MSG("not be the sub Camera");
		 	   continue;
	     	}
		#endif /*VENDOR_EDIT*/
...
)
{
	...
	char value[32] = {'\0'};
	ISP_DEBUG_INFO stISPDebugInfo;
	property_get("persist.isp.tuning.debug.enable", value, "0");
	stISPDebugInfo.ISPTuningEnable = atoi(value);
	property_get("persist.isp.tuning.capture.raw", value, "0");
	stISPDebugInfo.ISPRawDataEnable = atoi(value);
	property_get("persist.isp.printk.enable", value, "0");
	stISPDebugInfo.ISPDebugEnable = atoi(value);
	
    ret = ioctl(m_fdSensor, KDIMGSENSORIOC_X_SET_ISP_DEBUG_FLAG, &stISPDebugInfo);
	if (ret < 0) 
	{
		LOG_ERR("[init]: KDIMGSENSORIOC_X_SET_ISP_DEBUG_FLAG error\n");
...
{ 
    ...
    case CMD_SENSOR_GET_YUV_DELAY_INFO:
        ...
        break;
    #ifdef VENDOR_EDIT
    //LiuBin@MtkCamera, 2013/03/06, Add for isp flash capture sync
	case CMD_SENSOR_GET_ISP_STATUS:
		FeatureId = SENSOR_FEATURE_GET_ISP_STATUS;
        pu4FeaturePara = (MUINT32*)parg1;
        FeatureParaLen = sizeof(MUINT32);
        pFeaturePara = (MUINT8*)pu4FeaturePara;
		LOG_MSG("CMD_SENSOR_GET_ISP_STATUS--------- \r\n");
		break;
	//LiuBin@MtkCamera, 2013/03/12, Add for set isp flash mode
	case CMD_SENSOR_SET_FLASH_MODE:
		FeatureId = SENSOR_FEATURE_SET_FLASHLIGHT;
       	u4FeaturePara[0] = (MUINT32)parg1;
        FeatureParaLen = sizeof(MUINT32);
        pFeaturePara = (MUINT8*)&u4FeaturePara[0];
		LOG_MSG("CMD_SENSOR_SET_FLASH_MODE--------- \r\n");
		break;
	//feng.hu@mtk camera team modified for ms2r continuous af
    case CMD_SENSOR_DO_YUV_CAF:
        FeatureId = SENSOR_FEATURE_DO_YUV_CAF;
		pu4FeaturePara = (MUINT32*)parg1;
        FeatureParaLen = sizeof(MUINT32);
        pFeaturePara = (MUINT8*)pu4FeaturePara;
		LOG_MSG("CMD_SENSOR_DO_YUV_CAF------------\r\n");
        break;
	//ZhangZhengrong@MtkCamera, 2013/03/19, Add for scene detect
	case CMD_SENSOR_GET_WDV:
	    LOG_MSG("---------CMD_SENSOR_GET_WDV---------\r\n");
        FeatureId = SENSOR_FEATURE_GET_WDV;
        pu4FeaturePara = (MUINT32*)parg1;
        FeatureParaLen = sizeof(MUINT32);
        pFeaturePara = (MUINT8*)pu4FeaturePara;
        break;
	case CMD_SENSOR_GET_BRIGHTNESS:
		LOG_MSG("---------CMD_SENSOR_GET_BRIGHTNESS---------\r\n");
        FeatureId = SENSOR_FEATURE_GET_BRIGHTNESS;
        pu4FeaturePara = (MUINT32*)parg1;
        FeatureParaLen = sizeof(MUINT32);
        pFeaturePara = (MUINT8*)pu4FeaturePara;
        break;
	case CMD_SENSOR_GET_WB:
        FeatureId = SENSOR_FEATURE_GET_WB;
        pu4FeaturePara = (MUINT32*)parg1;
        FeatureParaLen = sizeof(MUINT32);
        pFeaturePara = (MUINT8*)pu4FeaturePara;
        break;
    case CMD_SENSOR_GET_AE_GAIN_SHUTTER:
        LOG_MSG("---------CMD_SENSOR_GET_AE_GAIN_SHUTTER---------\r\n");
        FeatureId = SENSOR_FEATURE_GET_AE_GAIN_SHUTTER;
        pu4FeaturePara = (MUINT32*)parg1;
        FeatureParaLen = sizeof(MUINT32);
        pFeaturePara = (MUINT8*)pu4FeaturePara;
        break;
    case CMD_SENSOR_GET_AF_POSITION:
        LOG_MSG("---------CMD_SENSOR_GET_AF_POSITION---------\r\n");
        FeatureId = SENSOR_FEATURE_GET_AF_POSITION;
        pu4FeaturePara = (MUINT32*)parg1;
        FeatureParaLen = sizeof(MUINT32);
        pFeaturePara = (MUINT8*)pu4FeaturePara;
        break;
	...
	case CMD_SENSOR_CMD_SILNET_PREVIEW:
		FeatureId = SENSOR_FEATURE_SILNET_PREVIEW;
		LOG_MSG("CMD_SENSOR_CMD_SILNET_PREVIEW--------- \r\n");
		break;
	#endif /* QUICK_CAPTURE */
	//LiuBin@MtkCamera, 2013/04/23, Add for set isp to capture
	case CMD_SENSOR_CMD_SET_ISP_CAPTURE:
		FeatureId = SENSOR_FEATURE_SET_ISP_CAPTURE;
		LOG_MSG("SENSOR_FEATURE_SET_ISP_CAPTURE--------- \r\n");
		break;
    #endif /* VENDOR_EDIT */
...
)
{
    ...
    if(SENSOR_FEATURE_SINGLE_FOCUS_MODE == FeatureId || SENSOR_FEATURE_CANCEL_AF == FeatureId 
		...
		#ifdef QUICK_CAPTURE	  
		//lanhe@MTKCamera,2013/03/14 ,add for silent preview sensor		
		|| SENSOR_FEATURE_SILNET_PREVIEW == FeatureId
		#endif /*QUICK_CAPTURE*/
		//LiuBin@MtkCamera, 2013/04/23, Add for set isp to capture
		|| SENSOR_FEATURE_SET_ISP_CAPTURE == FeatureId
		#endif /*VENDOR_EDIT*/
		){
