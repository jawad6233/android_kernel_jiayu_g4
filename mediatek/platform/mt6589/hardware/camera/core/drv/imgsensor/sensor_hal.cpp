    ...
    int arg3)
{
	...
	case SENSOR_CMD_GET_ISP_STATUS:
		cmdId = CMD_SENSOR_GET_ISP_STATUS;
		pDeviceDrv->sendCommand((SENSOR_DEV_ENUM) sensorDevId, cmdId, (MUINT32 *)arg1);
		LOG_MSG("SENSOR_CMD_GET_ISP_STATUS------\r\n");
		break;
	//LiuBin@MtkCamera, 2013/03/12, Add for set isp flash mode
	case SENSOR_CMD_SET_FLASH_MODE:
		cmdId = CMD_SENSOR_SET_FLASH_MODE;
		pDeviceDrv->sendCommand((SENSOR_DEV_ENUM) sensorDevId, cmdId, (MUINT32 *)arg1);
		LOG_MSG("SENSOR_CMD_GET_ISP_STATUS------\r\n");
		break;
	//feng.hu@mtk camera team modified for ms2r continuous af
    case SENSOR_CMD_DO_YUV_CAF:
        cmdId = CMD_SENSOR_DO_YUV_CAF;
        pDeviceDrv->sendCommand((SENSOR_DEV_ENUM)sensorDevId,cmdId, (MUINT32 *) arg1);          
        break; 
	//ZhangZhengrong@MtkCamera, 2013/03/19, Add for scene detect
	case SENSOR_CMD_GET_WDV:
        cmdId = CMD_SENSOR_GET_WDV;
        pDeviceDrv->sendCommand((SENSOR_DEV_ENUM)sensorDevId,cmdId, (MUINT32 *) arg1);
        break;
    case SENSOR_CMD_GET_BRIGHTNESS:
        cmdId = CMD_SENSOR_GET_BRIGHTNESS;
        pDeviceDrv->sendCommand((SENSOR_DEV_ENUM)sensorDevId,cmdId, (MUINT32 *) arg1);
        break;
    case SENSOR_CMD_GET_WB:
        cmdId = CMD_SENSOR_GET_WB;
        pDeviceDrv->sendCommand((SENSOR_DEV_ENUM)sensorDevId,cmdId, (MUINT32 *) arg1);
        break;
    case SENSOR_CMD_GET_AE_GAIN_SHUTTER:
        cmdId = CMD_SENSOR_GET_AE_GAIN_SHUTTER;
        pDeviceDrv->sendCommand((SENSOR_DEV_ENUM)sensorDevId,cmdId, (MUINT32 *) arg1);
        break;
    case SENSOR_CMD_GET_AF_POSITION:
        cmdId = CMD_SENSOR_GET_AF_POSITION;
        pDeviceDrv->sendCommand((SENSOR_DEV_ENUM)sensorDevId,cmdId, (MUINT32 *) arg1);
        break; 
	#ifdef QUICK_CAPTURE
	//lanhe@MTKCamera,2013/03/14 ,add for silent preview sensor 
	case SENSOR_CMD_SILNET_PREVIEW:
		cmdId = CMD_SENSOR_CMD_SILNET_PREVIEW;
		pDeviceDrv->sendCommand((SENSOR_DEV_ENUM) sensorDevId, cmdId);
		break;
	#endif /* QUICK_CAPTURE */
	//LiuBin@MtkCamera, 2013/04/23, Add for set isp to capture
	case SENSOR_CMD_SET_ISP_CAPTURE:
		cmdId = CMD_SENSOR_CMD_SET_ISP_CAPTURE;
		pDeviceDrv->sendCommand((SENSOR_DEV_ENUM) sensorDevId, cmdId);
		break;
    #endif /* VENDOR_EDIT */	
    default:
