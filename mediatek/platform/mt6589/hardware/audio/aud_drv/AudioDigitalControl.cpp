...
uint32 AudioDigitalControl::DlPolicyByDevice(uint32_t Device)
{
    ...
    if (Device & AUDIO_DEVICE_OUT_BLUETOOTH_SCO ||
        ...
        return AudioDigitalType::DAI_BT;
  		...
  		#ifdef EXTERNAL_CODEC_SUPPORT
         #ifndef VENDOR_EDIT
         //Jirui.kang@BasicDrv.Audio, 2013/02/17, Modify for external codec
             Device & AUDIO_DEVICE_OUT_FM_TX) {
         #else /* VENDOR_EDIT */
       
             Device & AUDIO_DEVICE_OUT_FM_TX ||
             (mStreamType == 3 && 
             (Device&0x04 || Device&0x08))) {
		#endif /* VENDOR_EDIT */
		#else /*EXTERNAL_CODEC_SUPPORT*/
		   Device & AUDIO_DEVICE_OUT_FM_TX) {	 
         #endif //EXTERNAL_CODEC13009_SUPPORT
...
#ifdef EXTERNAL_CODEC_SUPPORT       
//Jirui.kang@BasicDrv.Audio, 2013/02/17, Add for external codec
void AudioDigitalControl::setStreamType(int streamType)
{
    ALOGD("external codec test +%s(),streamType = %d\n", __FUNCTION__, streamType);
