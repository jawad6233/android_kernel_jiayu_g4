...
#ifndef VENDOR_EDIT
//GuoLing@Prd.AudioRouting, 2013/03/23, Modify for Fm speaker,fix me
...
#endif  /*VENDOR_EDIT*/
    #ifndef VENDOR_EDIT
    //Xianfeng.Fu@MultimediaSrv.VideoCodec,2013/03/18,Add for set input device when in ringtone mode
    if (mAudioMode == AUDIO_MODE_RINGTONE){
        ALOGD("%s(), set input device when in ringtone mode", __FUNCTION__);
        
        if (mDlOutputDevice & AUDIO_DEVICE_OUT_WIRED_HEADSET){
            setUlInputDevice(AUDIO_DEVICE_IN_WIRED_HEADSET);
        } 
        else if ((mDlOutputDevice & AUDIO_DEVICE_OUT_EARPIECE)
                ||(mDlOutputDevice & AUDIO_DEVICE_OUT_SPEAKER)
                || (mDlOutputDevice & AUDIO_DEVICE_OUT_WIRED_HEADPHONE)){
                setUlInputDevice(AUDIO_DEVICE_IN_BUILTIN_MIC);
        }
        else if ((mDlOutputDevice & AUDIO_DEVICE_OUT_BLUETOOTH_SCO)
                ||(mDlOutputDevice & AUDIO_DEVICE_OUT_BLUETOOTH_SCO_HEADSET)
                || (mDlOutputDevice & AUDIO_DEVICE_OUT_BLUETOOTH_SCO_CARKIT)){
...
/* xiang.fei@BasicDrv.Audio, 2012/12/22, Add for Audio */
    ...
    if(IsWiredHeadsetOn())
    {
        mAudioAnalogInstance->setParameters(1, 0, 0);
    }
    else
    {
        mAudioAnalogInstance->setParameters(0, 0, 0);
    }
#endif
#endif /* VENDOR_EDIT */
...
#ifdef VENDOR_EDIT
//GuoLing@Prd.AudioRouting, 2013/04/27, Modify for set iMode -1 IN_CALL
#ifdef OPPO_WOLFSON_CODEC_SUPPORT
            setParameters(-1, 0);
...
#ifdef VENDOR_EDIT
//GuoLing@Prd.AudioRouting, 2013/04/27, Modify for set iMode -1 COMMUNICATION
#ifdef OPPO_WOLFSON_CODEC_SUPPORT
            setParameters(-1, 0);
#endif /* OPPO_WOLFSON_CODEC_SUPPORT */
#endif /* VENDOR_EDIT */
...
/* xiang.fei@BasicDrv.Audio, 2012/12/22, Add for Audio */
#ifdef OPPO_WOLFSON_CODEC_SUPPORT
mAudioAnalogInstance->setParameters(command1, 0);
#endif
#endif /* VENDOR_EDIT */
