...
/* hanbin.lin@BasicDrv.Audio, 2013/05/08, Add for Audio parameters */
int AudioMachineDevice::GetParameterId()
{
    int ret;
      ::ioctl(mFd, GET_AUDIO_PARAMETER_VERSION, &audio_parameter_id);
    ret = audio_parameter_id;
    ALOGD("audio_ParameterId ret = %d!!!!!!!!!!!!!",ret);
    return ret;
}
...
/* xiang.fei@BasicDrv.Audio, 2012/12/22, Add for Audio */
    ...
    iMode = audio_mode_t(-1);
    iHeadsetState = 0;
    bExtCodec_Path = false;
    bAnalog_Path = false;
//GuoLing@Prd.AudioRouting, 2013/04/02,add for ringing
    bSpeakAndHeadset = false;
#endif
#endif /* VENDOR_EDIT */
#ifdef VENDOR_EDIT
#ifdef EXTERNAL_CODEC_SUPPORT
/* hanbin.lin@BasicDrv.Audio, 2013/05/08, Add for Audio parameters */
    audio_parameter_id = 2;
#endif /*EXTERNAL_CODEC_SUPPORT*/
#endif /*VENDOR_EDIT*/
...
#ifdef EXTERNAL_CODEC_SUPPORT
/* hanbin.lin@BasicDrv.Audio, 2013/05/08, Add for Audio parameters */
    ALOGW("AudioMachineDevice init GET_AUDIO_PARAMETER_VERSION 11111111111111111=  %d!!!!!!!!!!", audio_parameter_id);
    ::ioctl(mFd, GET_AUDIO_PARAMETER_VERSION, &audio_parameter_id);
    ALOGW("AudioMachineDevice init GET_AUDIO_PARAMETER_VERSION =  %d!!!!!!!!!!", audio_parameter_id);
#endif /*EXTERNAL_CODEC_SUPPORT*/
#endif /*VENDOR_EDIT*/
...
status_t AudioMachineDevice::SetHeadPhoneGain(AudioAnalogType::VOLUME_TYPE volume_Type, int volume)
{
    ...
    index  += (volume);
#ifndef VENDOR_EDIT
//Jirui.kang@BasicDrv.Audio, 2013/03/15, Modify for mic to speaker loopbacktest
if(index > 12){
#else /* VENDOR_EDIT */
    if(index > 12 && 15 != index){ 
#endif /* VENDOR_EDIT */
        index = 12;
    }
...
{
    ALOGD("AudioMachineDevice AnalogOpen DeviceType = %s\n", kAudioAnalogDeviceTypeName[DeviceType]);
...
/* hanbin.lin@BasicDrv.Audio, 2013/05/08, Add for Audio parameters */
    ...
    ::ioctl(mFd, GET_AUDIO_PARAMETER_VERSION, &audio_parameter_id);
    ALOGD("AudioMachineDevice audio_parameter_id GET_AUDIO_PARAMETER_VERSION = %d !!!!!!!!!!!!\n", audio_parameter_id);
    GetParameterId();
    ALOGD("AudioMachineDevice audio_parameter_id = %d !!!!!!!!!!!!\n", audio_parameter_id);
#endif /*EXTERNAL_CODEC_SUPPORT*/
#endif /*VENDOR_EDIT*/
...
/* xiang.fei@BasicDrv.Audio, 2012/12/22, Add for Audio */
#ifdef OPPO_WOLFSON_CODEC_SUPPORT
    ALOGD("AudioMachineDevice AnalogOpen iMode = %d bExtCodec_Path=%d\n", iMode, bExtCodec_Path);
    if((iMode == android_audio_legacy::AudioSystem::MODE_NORMAL) && !bExtCodec_Path 
    && ((DeviceType == AudioAnalogType::DEVICE_OUT_SPEAKERR) || (DeviceType == AudioAnalogType::DEVICE_OUT_HEADSETR))
    && !bSpeakAndHeadset)
    {
        bExtCodec_Path = true;
        if(iHeadsetState)
        {
            /* open extern codec HP DAC path */
            ::ioctl(mFd, SET_EXTERN_CODEC_HP, 1);
        }
        else
        {
            /* open extern codec SPK DAC path */
            ::ioctl(mFd, SET_EXTERN_CODEC_SPK, 1);
        }
    }
    else
    {   
        if((DeviceType == AudioAnalogType::DEVICE_OUT_EARPIECER) || (DeviceType == AudioAnalogType::DEVICE_OUT_SPEAKERR)
        || (DeviceType == AudioAnalogType::DEVICE_OUT_HEADSETR))
        {
            bAnalog_Path = true; 
        }
#endif /* OPPO_WOLFSON_CODEC_SUPPORT */
#endif /* VENDOR_EDIT */
        switch (DeviceType) {
        ...
        #ifdef VENDOR_EDIT
        #ifdef EXTERNAL_CODEC_SUPPORT
        //Jirui.kang@BasicDrv.Audio, 2013/03/12, Add for pop noise
            usleep(50*1000);
        #endif
        #endif /* VENDOR_EDIT */
        
        #ifndef VENDOR_EDIT
        /* xiang.fei@BasicDrv.Audio, 2012/12/22, Modify for Headset */
            ioctl(mFd,SET_HEADPHONE_ON,NULL); // tell kernel to open device   
        #else
            ioctl(mFd,SET_HEADSET_STATE, 1);
        #endif /* VENDOR_EDIT */
            break;
        ...
        case AudioAnalogType::DEVICE_OUT_SPEAKERL:
            #ifdef USING_EXTAMP_HP
                #ifdef VENDOR_EDIT
                /* xiang.fei@BasicDrv.Audio, 2013/03/11, Modify for Audio */
                //GuoLing@Prd.AudioRouting, 2013/03/30,Modify for enable headset but not open when speaker on
                //#ifdef OPPO_WOLFSON_CODEC_SUPPORT
                DeviceType = AudioAnalogType::DEVICE_OUT_HEADSETR;
                mBlockAttribute[DeviceType].mEnable = true;
                mAudioAnalogReg->SetAnalogReg(AFUNC_AUD_CON2, 0x0080, 0x0080);
...
{
    ALOGD("SetFrequency frequency %d", frequency);
...
/* xiang.fei@BasicDrv.Audio, 2012/12/22, Add for Audio */
    ...
    if((iMode == android_audio_legacy::AudioSystem::MODE_NORMAL) && !bExtCodec_Path)
    {   
        ALOGD("DAC frequency %d", frequency);
        ::ioctl(mFd, SET_EXTERN_CODEC_FREQ, frequency);
    }
#endif /* OPPO_WOLFSON_CODEC_SUPPORT */
#endif /* VENDOR_EDIT */
...
/* xiang.fei@BasicDrv.Audio, 2012/12/22, Add for Audio */
#ifdef OPPO_WOLFSON_CODEC_SUPPORT
    ALOGD("AudioMachineDevice AnalogClose iMode = %d bExtCodec_Path=%d\n", iMode, bExtCodec_Path);
    if(bExtCodec_Path && ((DeviceType == AudioAnalogType::DEVICE_OUT_SPEAKERR) || (DeviceType == AudioAnalogType::DEVICE_OUT_HEADSETR)))
    {
        bExtCodec_Path = false;
        iMode = audio_mode_t(-1);
        if(iHeadsetState)
        {
            /* close extern codec HP DAC path */
            ::ioctl(mFd, SET_EXTERN_CODEC_HP, 0);
        }
        else
        {
            /* close extern codec SPK DAC path */
            ::ioctl(mFd, SET_EXTERN_CODEC_SPK, 0);
        }
    }
    else
    {
...
/* xiang.fei@BasicDrv.Audio, 2012/12/22, Add for Audio */
#ifdef OPPO_WOLFSON_CODEC_SUPPORT
    iHeadsetState = command1;
    ALOGD("setParameters iHeadsetState=%d", iHeadsetState);
#endif
#endif /* VENDOR_EDIT */
...
/* xiang.fei@BasicDrv.Audio, 2012/12/22, Add for Audio */
#ifdef OPPO_WOLFSON_CODEC_SUPPORT  
bool AudioMachineDevice::getExtCodecPathStatus(void)
{
    return bExtCodec_Path;
}

bool AudioMachineDevice::getAnalogPathStatus(void)
{
    return bAnalog_Path;
}
#endif
