...
//Jirui.kang@BasicDrv.Audio, 2013/02/21, Add for switch afe when streamtype have changed
    static uint32 sDigitalPart = AudioDigitalType::I2S_OUT_DAC;
    ...
    if (GetStreamRunning() == false) {
        ...
        switch (Mode) {
            ...
            //Jirui.kang@BasicDrv.Audio, 2013/02/21, Add for switch afe when streamtype have changed 
            #ifdef EXTERNAL_CODEC_SUPPORT
                ALOGD("external codec test %s(), DigitalPart = %d", __FUNCTION__,DigitalPart);
                sDigitalPart = DigitalPart;
            #endif //EXTERNAL_CODEC_SUPPORT  
            #endif /* VENDOR_EDIT */
    ...
    switch (Mode) {
        ...
        //Jirui.kang@BasicDrv.Audio, 2013/02/21, Add for switch afe when streamtype have changed 
        #ifdef EXTERNAL_CODEC_SUPPORT
            if(DigitalPart != sDigitalPart) {
                if( mAudioDigitalControl->GetI2SConnectStatus() )
                    TurnOffAfeDigital(sDigitalPart, true);
                else
                    TurnOffAfeDigital(sDigitalPart, false);
                TurnOnAfeDigital(DigitalPart);
            }
            sDigitalPart = DigitalPart; 
        #endif //EXTERNAL_CODEC_SUPPORT      
...
/* xiang.fei@BasicDrv.Audio, 2012/12/22, Add for Audio */
    ...
    int value = 0;
    static String8 keyStreamTypeMusic = String8("SetStreamTypeMusic");

    if (param.getInt(keyStreamTypeMusic, value) == NO_ERROR && !mAudioAnalogControl->GetFmStatus()) 
        { 
            param.remove(keyStreamTypeMusic);
            if(value)
            {
                mAudioResourceManager->setParameters(0, 0);
                audio_mode_t Mode_new = mAudioResourceManager->GetAudioMode();
                if((GetStreamRunning() && (mAudioAnalogControl->getAnalogPathStatus()) && (Mode_new == AUDIO_MODE_NORMAL))
                    || mAudioAnalogControl->GetFmStatus())
                {
                    mAudioResourceManager->StopOutputDevice();
                    usleep(5*1000);
                    if(GetStreamRunning())
            ...
            {
                if(mAudioAnalogControl->GetFmStatus())
                {
                    mAudioResourceManager->StopOutputDevice();
                    usleep(5*1000);
                    mAudioResourceManager->StartOutputDevice();
...
//Jirui.kang@BasicDrv.Audio, 2013/02/17, Add for external codec
    String8 keySetStreamType = String8(AudioParameter::keySetStreamType);
    int value = 0;
    if (param.getInt(keySetStreamType, value) == NO_ERROR) {
        param.remove(keySetStreamType);
        mAudioDigitalControl->setStreamType(value);
        ALOGD("external codec test setParameters() keyStreamType = %d", value);
    }
